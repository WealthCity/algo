...
from securities_master.retrieve_data import retrieve_data as rd
...
class HistoricCSVDataHandler(DataHandler): 
	...
	def __init__(self, events, csv_dir, symbol_list): 
		...
		Parameters: events - The Event Queue. csv_dir - Absolute directory path to the CSV files. symbol_list - A list of symbol strings. """ 
		self.events = events 
	...
	def _open_convert_csv_files(self): 
		...
		# Load the CSV file with no header information, indexed on date 
			self.symbol_data[s] = pd.io.parsers.read_csv( os.path.join(self.csv_dir, '{}.csv'.format(s)), header=0, index_col=0, parse_dates=True, 
...
class DatabaseDataHandler(DataHandler): 
	...
	def _get_data_from_db(self):		
		for symbol in self.symbol_list:
			self.symbol_data[symbol]=rd.get_data_from_ticker(symbol)
...
if __name__ =='__main__':
	data_handler_from_data_base = DatabaseDataHandler(event,['CBA','1PG'])
